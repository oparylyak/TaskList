...
export class TaskListService {
    ...
    private $q:IQService;
    private tasks;
    private macIP = 'http://192.168.0.106:3000';
    ...
    getTasks() {
        ...
        if(this.tasks){
            ...
            this.$http.get( 'http://192.168.0.106:3000/taskList').then( (response)=> {
                ...
                // handle errors
                console.log(111,'Get Post Error')
            });;
        }
    ...
    createTask(task) {
        ...
        task.resolved = false;
        this.$http.post(this.macIP + '/createTask',task).then( (response)=> {
    ...
    deleteTask(task) {
        ...
        this.$http.delete(this.macIP + '/taskList/'+ task._id).then( (response) =>{
            this.getTasks();
        });
   ...
   updateTask(task) {
       ...
       this.$http.put(this.macIP + '/taskList/' + task._id, task).then( (response)=> {
        });
